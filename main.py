...
def start_message(message):
    ...
    for i in range(len(data)):
        if data[i][0] == str(message.chat.id):
            check = 1
    if  check == 0: cursor.execute("INSERT INTO member (id, step1, step2) VALUES(%s, 2, 2);", [message.chat.id])
...
def send_text(message):
    ...
    if message.text == 'Выбрать тематику задач':
        ...
        bot.send_message(message.chat.id, text)
        cursor.execute("UPDATE member SET step2 = 1 WHERE id = %s;", [str(message.chat.id)])
    ...
    if message.text != 'Выбрать тематику задач' and message.text != 'Выбрать сложность задач':
        ...
        data = cursor.fetchall()
        cursor.execute("SELECT step2 FROM member WHERE id = %s;", [str(message.chat.id)])
        ...
        if data2[0][0] == 1:
            ...
            for i in range(len(data)):
                if data[i][0] == message.text:
                    check = 1
            if check == 1:
                cursor.execute("UPDATE member SET step2 = 0 WHERE id = %s;", [str(message.chat.id)])
        ...
        if message.text == 'Получить задания':
            ...
            if data[0][0] and data2[0][0]:
                ...
                bot.send_message(message.chat.id, "Не выбрано сложность или темматика задачи.")
